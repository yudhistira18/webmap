...
st.title("🔍 Komposit dan Visualisasi Data Eksplorasi")
...
progress.progress(25, text="🔁 Proses compositing per BHID dan Layer...")
...
# Total depth
progress.progress(50, text="📏 Menghitung total depth...")
depth = df.groupby('BHID')['To'].max().reset_index()
depth.columns = ['BHID', 'Total_Depth']
composite_df = composite_df.merge(depth, on='BHID', how='left')
...
# ===============================
# 4. Konversi Koordinat UTM ke WGS84
# ===============================
...
progress.progress(85, text="🧱 Menyusun tampilan data...")
...
AgGrid(composite_table, gridOptions=grid_options, theme="streamlit", height=400, fit_columns_on_grid_load=True)

st.download_button(
    label="⬇️ Download CSV (Filtered)",
    data=composite_table.to_csv(index=False).encode(),
    file_name="composite_filtered.csv",
    mime="text/csv"
)

# ===============================
# 8. Tabel Total Kedalaman
# ===============================
st.markdown("### 📏 Tabel Total Depth")

depth_filtered = depth[depth["BHID"].isin(filtered_df["BHID"].unique())].copy()
...
# ===============================
# 9. Tabel Koordinat UTM + Total Depth
# ===============================
st.markdown("### 📍 Tabel Koordinat Collar (UTM) dan Total Kedalaman")

coord_table = filtered_df[['BHID', 'XCollar', 'YCollar', 'ZCollar']].drop_duplicates()
coord_table = coord_table.merge(depth, on='BHID', how='left')
